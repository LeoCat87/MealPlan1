...
with st.sidebar:
    ...
    # PROFILO: crea PRIMA, poi disegna selectbox (evita conflitti)
    st.caption("Profilo")
    np_c1, np_c2 = st.columns([2,1])
    with np_c1:
        new_profile_name = st.text_input("Nuovo profilo", key="new_profile_name", placeholder="Es. Famiglia", label_visibility="collapsed")
    with np_c2:
        if st.button("Crea"):
